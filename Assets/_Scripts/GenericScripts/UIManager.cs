...
public class UIManager : MonoBehaviour
{
    ...
    public GameObject playerLevelCount;
    public GameObject levelEndCount;
    ...
    private void PrepareGameName()
    {
        ...
        gameNameParentGameObject.transform.GetChild(0).GetComponent<TextMeshProUGUI>().outlineWidth = 0.2f;
    ...
    public void LevelCompleteForUI(float delay, int score)
    {
        if (!isLevelCompletedForUI)
        {
            StartCoroutine(WaitForLevelComplete(delay, score));

            isLevelCompletedForUI = true;
        }
    }
    IEnumerator WaitForLevelComplete(float delay, int score)
    {
    ...
    private void HandleLevelEnd(int score)
    {
        Sequence levelEndCountSeq = DOTween.Sequence();
        Tween scaleTween = levelEndCount.GetComponent<RectTransform>().DOScale(1.5f, .3f)
            .OnStart(delegate
            {
                levelEndCount.GetComponent<TextMeshProUGUI>().text = $"X{score}";
                levelEndCount.SetActive(true);
            })
            .SetLoops(2, LoopType.Yoyo);

        Tween moveTween = levelEndCount.transform.DOMove(gemCountInLevelTexts[1].transform.position, .3f)
            .OnComplete(delegate
            {
                levelEndCount.gameObject.SetActive(false);
                GameManager.instance.gemCountInLevel *= score;
                gemCountInLevelTexts[1].GetComponent<TextMeshProUGUI>().text = GameManager.instance.gemCountInLevel.ToString();
                GameManager.instance.gemCountTotal += GameManager.instance.gemCountInLevel;
                GameManager.instance.SaveCountTotal();
            });

        float counter = 0f;

        Tween waitTween = DOTween.To(() => counter, x => x = counter, 1f, .5f)
            .OnComplete(delegate
            {
                CountGems();
            });

        levelEndCountSeq.Append(scaleTween);
        levelEndCountSeq.Append(moveTween);
        levelEndCountSeq.Append(waitTween);


        levelEndCountSeq.Play();
    }
